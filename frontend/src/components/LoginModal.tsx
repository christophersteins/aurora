...
export default function LoginModal({ isOpen, onClose, onSwitchToRegister, onSwitchToForgotPassword }: LoginModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const errorMessage = err instanceof Error
        ? err.message
        : (err as { response?: { data?: { message?: string } } })?.response?.data?.message || t('loginFailed');
  ...
  const handleSocialLogin = (provider: string) => {
    console.log(`${provider} Login - will be implemented later`);
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-page-primary">
      ...
      <button
        onClick={onClose}
        className="absolute top-6 right-6 p-2 text-[var(--text-button)] hover:text-[#8b5cf6] transition cursor-pointer"
        aria-label="Close"
      >
        <X size={32} />
      </button>
      ...
      <div className="w-full max-w-md">
        ...
        <div className="relative bg-page-primary rounded-lg p-8 border border-[#2f3336]" style={{
          ...
          <h2 className="text-2xl mb-6 text-heading">{t('loginButton')}</h2>
          ...
          <div className="space-y-3 mb-6">
            ...
            <button
              ...
              className="w-full btn-base btn-secondary flex items-center justify-center gap-3 cursor-pointer"
            >
              <svg className="w-5 h-5" viewBox="0 0 24 24" fill="currentColor">
                <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/>
              </svg>
              <span>{t('loginWith', { provider: 'X' })}</span>
            </button>
            ...
            >
              ...
              <span>{t('loginWith', { provider: 'Apple' })}</span>
            </button>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="relative">
              ...
              <button
                ...
                className="absolute right-3 top-1/2 -translate-y-1/2 text-muted hover:text-body transition cursor-pointer"
            ...
            <div className="text-right">
              <button
                ...
                className="text-sm text-muted hover:text-body hover:underline transition cursor-pointer"
              >
                {t('forgotPassword')}
              </button>
            </div>
          ...
          <div className="mt-6 text-center">
            <p className="text-muted">
              ...
              <button
                ...
                className="link-default cursor-pointer"
              >
                {t('registerNow')}
              </button>
            </p>
